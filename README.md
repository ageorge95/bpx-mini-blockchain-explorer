...
- The tool:
  - can scrape the blocks data via geth's RPC and extract transactions and block rewards for a given eth address

- The tool needs:
  - a running and synced geth node
...
### Installation
```
git clone https://github.com/ageorge95/bpx-mini-blockchain-explorer bpx_explorer
cd bpx_explorer
sh install.sh
```
...
- The script will use the web3 library and geth's RPC interface and scrape all the blockchain blocks
- On subsequent executions it will skip already scraped blocks - it uses a cache mechanism
- The output file will be an excel document containing all your transactions and block rewards
